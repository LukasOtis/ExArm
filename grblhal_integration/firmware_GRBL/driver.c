...
#endif
#ifdef AUXINPUT8_PIN
    { .id = Input_Aux8, .port = GPIO_INPUT, .pin = AUXINPUT8_PIN, .group = PinGroup_AuxInput },
#endif
#ifdef AUXINPUT9_PIN
    { .id = Input_Aux9, .port = GPIO_INPUT, .pin = AUXINPUT9_PIN, .group = PinGroup_AuxInput },
#endif
...
static output_signal_t outputpin[] = {
    ...
    { .id = Output_Analog_Aux0, .port = GPIO_OUTPUT, .pin = AUXOUTPUT0_PWM_PIN, .group = PinGroup_AuxOutputAnalog, .mode = { .pwm = On } },
#endif
#ifdef AUXOUTPUT1_PWM_PIN
    { .id = Output_Analog_Aux1, .port = GPIO_OUTPUT, .pin = AUXOUTPUT1_PWM_PIN, .group = PinGroup_AuxOutputAnalog, .mode = { .pwm = On } },
#endif
#ifdef AUXOUTPUT2_PWM_PIN
    { .id = Output_Analog_Aux2, .port = GPIO_OUTPUT, .pin = AUXOUTPUT2_PWM_PIN, .group = PinGroup_AuxOutputAnalog, .mode = { .pwm = On } },
#endif
...
static void probeConfigure (bool is_probe_away, bool probing)
{
    ...
    gpio_set_inover(((input_signal_t *)probe->input)->pin, probe_state.inverted ? GPIO_OVERRIDE_INVERT : GPIO_OVERRIDE_NORMAL);
...
static probe_state_t probeGetState (void)
{
    ...
    if(probe_state.is_probing && probe_state.irq_enabled)
        ...
        state.triggered = DIGITAL_IN(((input_signal_t *)probe->input)->pin);
...
void settings_changed (settings_t *settings, settings_changed_flags_t changed)
{
    ...
    if (IOInitDone) {
        ...
        do {
            ...
            gpio_init(input->pin); // TODO: only run once at startup?
